...
for (job_index in seq_along(job_names)) {
  ...
  # Can use mset$msgSet$replace.msg to  view the small value that replaced the missing/zero values)
  ...
  ##############
  # Perform Fold-change Analysis PRIOR to Normalization
  ##############
  # "The goal of fold-change (FC) analysis is to compare the absolute value of
  # change between two group means. Since column-wise normalization (i.e. log
  # transformation, mean-centering) will significantly alter absolute values,
  # FC is calculated as the ratio between two group means using the data before
  # column-wise normalization was applied."
